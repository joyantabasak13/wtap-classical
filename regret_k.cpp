...
// Regret-k helper.
//
// For a target j, let the currently feasible weapon scores be sorted as
//
//   s(1) >= s(2) >= ... >= s(r).
//
// We use the standard accumulated regret-k form for a maximization problem:
//
//   R_k(j) = sum_{h=2}^{min(k,r)} [ s(1) - s(h) ].
//
// Thus k=2 reduces exactly to the original best-minus-second-best regret.
// If only one weapon type remains feasible (r=1), the fallback is treated
// as score 0, so R_k(j)=s(1), preserving the original implementation's
// behavior in the no-fallback case.
// ------------------------------------------------------------
struct RegretKInfo {
    int bestWeapon = -1;
...
RegretKInfo computeRegretK(
    ...
    if (regretK < 2)
        throw invalid_argument("regretK must be at least 2.");

    vector<pair<double,int>> feasible;
    feasible.reserve(w.size());

    for (int i = 0; i < static_cast<int>(w.size()); ++i) {
        if (w[i] <= 0) continue;
        feasible.emplace_back(S[i][target], i);
    }
    ...
    out.bestScore = feasible[0].first;
    out.bestWeapon = feasible[0].second;

    if (feasible.size() == 1) {
        out.regret = max(0.0, out.bestScore);
        return out;
    }

    const int depth = min(regretK, static_cast<int>(feasible.size()));
    long double r = 0.0L;
    for (int h = 1; h < depth; ++h)
        r += static_cast<long double>(out.bestScore - feasible[h].first);

    out.regret = max(0.0, static_cast<double>(r));
    return out;
}
...
//
// Define accumulated regret-k for sorted feasible scores
// s(1) >= s(2) >= ...:
//
//   regret_k(j) = sum_{h=2}^{min(k,r)} (s(1) - s(h))
//
// k=2 is exactly the original best-minus-second-best rule.
//
...
// ------------------------------------------------------------
Solution solveRegretCWTAA(const Instance& ins, int regretK = 2) {
    auto start = Clock::now();
    if (regretK < 2) throw invalid_argument("Regret-CWTAA regretK must be at least 2.");
    ...
    while (unitsLeft > 0 && targetsLeft > 0) {
        ...
        for (int j = 0; j < m; ++j) {
            ...
            const RegretKInfo info = computeRegretK(S, w, j, regretK);
            if (info.bestWeapon < 0)
                continue;

            const double bestScore = info.bestScore;
            const int bestWeapon = info.bestWeapon;
            const double regret = info.regret;
...
Solution constructRRFromScoreMatrix(
    ...
    const int n = ins.nTypes;
    const int m = ins.nTargets;
    if (regretK < 2) throw invalid_argument("RR regretK must be at least 2.");
    ...
    while (unitsLeft > 0 && targetsLeft > 0) {
        ...
        for (int j = 0; j < m; ++j) {
            ...
            Candidate c;
            c.target = j;
            c.weapon = info.bestWeapon;
            c.bestScore = info.bestScore;
            c.regret = info.regret;
            candidates.push_back(c);
        }
...
Solution runOneUnperturbedRR(
    ...
    Solution sol = constructRRFromScoreMatrix(ins, S, rng, topK, alpha, regretK);
...
Solution runOneTargetPermutedRR(
    ...
    Solution permutedSol =
        constructRRFromScoreMatrix(permuted, S, rng, topK, alpha, regretK);
...
Solution runOneInputPerturbedRR(
    ...
    Solution sol = constructRRFromScoreMatrix(
        original, perturbedS, rng, topK, alpha, regretK
    );
...
void runBenchmark(
    ...
    // Regret depth k_r: number of ranked weapon alternatives used to
    // compute accumulated regret.  This is DIFFERENT from RR_TOP_K,
    // which is the size of the randomized target candidate list.
    const vector<int> REGRET_K_VALUES = {2, 5, 10};
    ...
    cout << "Per instance:\n";
    cout << "  CWTAA:                     1 run\n";
    cout << "  Deterministic Regret-k:    k=2,5,10 (1 run each)\n";
    cout << "  Randomized Regret-k:       k=2,5,10 (19 runs each)\n";
    cout << "  Input-Perturbed RR-k:      18 runs per k=2,5,10 "
            "(3 each at 0.01%,0.05%,0.1%,0.5%,1%,5%)\n";
    cout << "  Target-Permuted RR-k:      10 runs per k=2,5,10\n";
    ...
    for (int c = 0; c < numCases; ++c) {
        ...
        // ----------------------------------------------------
        // 2) Deterministic Regret-k CWTAA: k_r = 2, 5, 10
        // ----------------------------------------------------
        for (int regretK : REGRET_K_VALUES) {
            Solution sol = solveRegretCWTAA(ins, regretK);
            addRow(
                "RegretKCWTAA",
                "DeterministicRegret_k" + to_string(regretK),
                0.0, 1, instanceSeed, sol
            );
        }

        // ----------------------------------------------------
        // 3) Unperturbed Randomized Regret-k: 19 runs per k_r.
        //    IMPORTANT: RR_TOP_K remains fixed at 5, so this isolates
        //    the effect of regret depth k_r from candidate-list size.
        // ----------------------------------------------------
        for (int regretK : REGRET_K_VALUES) {
            for (int r = 1; r <= UNPERTURBED_RR_RUNS; ++r) {
                // Use the SAME runSeed for the same restart number across
                // k_r values, which gives a cleaner paired comparison.
                const uint64_t runSeed =
                    instanceSeed
                    ^ (0xA24BAED4963EE407ULL *
                       static_cast<uint64_t>(r));

                Solution sol = runOneUnperturbedRR(
                    ins, runSeed, RR_TOP_K, RR_ALPHA, regretK
                );

                addRow(
                    "RandomizedRegretK",
                    "UnperturbedRR_RegretK" + to_string(regretK),
                    0.0,
                    r,
                    runSeed,
                    sol
        ...
        // ----------------------------------------------------
        // 4) INPUT-PERTURBED Randomized Regret-k:
        //    6 levels x 3 runs x k_r in {2,5,10}.
        //    The same seeds are reused across k_r values for paired
        //    comparison of regret depth.
        // ----------------------------------------------------
        for (int regretK : REGRET_K_VALUES) {
            for (size_t l = 0;
                ...
                for (int r = 1;
                    ...
                    Solution sol = runOneInputPerturbedRR(
                        ins, runSeed, pct, RR_TOP_K, RR_ALPHA, regretK
                    );

                    addRow(
                        "PerturbedRandomizedRegretK",
                        "InputPerturb_" + pctLabel(pct) +
                            "_RegretK" + to_string(regretK),
                        pct,
                        r,
                        runSeed,
                        sol
        ...
        // ----------------------------------------------------
        // 5) Target-permuted NON-PERTURBED Randomized Regret-k:
        //    10 target permutations for each k_r in {2,5,10}.
        // ----------------------------------------------------
        for (int regretK : REGRET_K_VALUES) {
            for (int r = 1; r <= TARGET_PERMUTATION_RR_RUNS; ++r) {
                ...
                Solution sol = runOneTargetPermutedRR(
                    ins, runSeed, RR_TOP_K, RR_ALPHA, regretK
                );

                addRow(
                    "PermutedRandomizedRegretK",
                    "TargetPermutationRR_RegretK" + to_string(regretK),
                    0.0,
                    r,
                    runSeed,
                    sol
        ...
        // ----------------------------------------------------
        // Extra combined summaries by regret depth k_r.
        // These DO NOT mix k_r values, so k=2,5,10 can be compared
        // directly under identical search budgets.
        // ----------------------------------------------------
        for (int regretK : REGRET_K_VALUES) {
            ...
            const string kTag = "RegretK" + to_string(regretK);

            for (const RunRecord& r : rows) {
                if (r.method == "RandomizedRegretK" &&
                    r.variant == "UnperturbedRR_" + kTag) {
                    unperturbedRR.push_back(&r);
                    allRandomizedSearch.push_back(&r);
                }
                else if (r.method == "PerturbedRandomizedRegretK" &&
                         r.variant.find("_" + kTag) != string::npos) {
                    allPerturbedRR.push_back(&r);
                    allRandomizedSearch.push_back(&r);
                }
                else if (r.method == "PermutedRandomizedRegretK" &&
                         r.variant == "TargetPermutationRR_" + kTag) {
            ...
            writeCaseVariant(
                "RandomizedRegretK",
                "UnperturbedRR_" + kTag + "_BestOf19",
                unperturbedRR
            );

            writeCaseVariant(
                "PerturbedRandomizedRegretK",
                "PerturbedRR_" + kTag + "_BestOf18",
                allPerturbedRR
            );

            writeCaseVariant(
                "PermutedRandomizedRegretK",
                "TargetPermutationRR_" + kTag + "_BestOf10",
                targetPermutedRR
            );

            writeCaseVariant(
                "CombinedRandomizedSearchK",
                "BestOf47_" + kTag + "_RR_Perturbed_Permuted",
                allRandomizedSearch
            );
        }
        ...
        map<int,double> bestUnpert, bestPert, bestPerm, regretCost;
        for (int k : REGRET_K_VALUES) {
            bestUnpert[k] = numeric_limits<double>::infinity();
            bestPert[k] = numeric_limits<double>::infinity();
            bestPerm[k] = numeric_limits<double>::infinity();
            regretCost[k] = numeric_limits<double>::infinity();
        }
        double cwtaaCost = numeric_limits<double>::infinity();
        double hungCost = numeric_limits<double>::infinity();

        for (const RunRecord& r : rows) {
            for (int k : REGRET_K_VALUES) {
                const string kTag = "RegretK" + to_string(k);
                if (r.method == "RandomizedRegretK" &&
                    r.variant == "UnperturbedRR_" + kTag)
                    bestUnpert[k] = min(bestUnpert[k], r.solution.totalSurvivalValue);
                else if (r.method == "PerturbedRandomizedRegretK" &&
                         r.variant.find("_" + kTag) != string::npos)
                    bestPert[k] = min(bestPert[k], r.solution.totalSurvivalValue);
                else if (r.method == "PermutedRandomizedRegretK" &&
                         r.variant == "TargetPermutationRR_" + kTag)
                    bestPerm[k] = min(bestPerm[k], r.solution.totalSurvivalValue);
                else if (r.method == "RegretKCWTAA" &&
                         r.variant == "DeterministicRegret_k" + to_string(k))
                    regretCost[k] = r.solution.totalSurvivalValue;
            }
        ...
        cout << "Case " << setw(2) << (c + 1)
             << " | CWTAA=" << fixed << setprecision(6) << cwtaaCost
             << " | DetR[k2,k5,k10]=[" << regretCost[2] << ","
             << regretCost[5] << "," << regretCost[10] << "]"
             << " | RR19[k2,k5,k10]=[" << bestUnpert[2] << ","
             << bestUnpert[5] << "," << bestUnpert[10] << "]"
             << " | Hungarian=" << hungCost
             << "\n";
    }
}
...
//
// Regret-depth experiment compares k_r = 2, 5, 10 while keeping RR_TOP_K fixed.
// ------------------------------------------------------------
